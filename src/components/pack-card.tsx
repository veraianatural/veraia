    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-[2rem] border border-moss/10 bg-white shadow-[0_18px_50px_rgba(47,58,45,.10)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_28px_80px_rgba(47,58,45,.16)]">
      <Link href={`/rituales/${pack.slug}`} className="block">
        <div className="relative h-80 overflow-hidden bg-sand/30">
          <ProductImage src={pack.image} alt={pack.name} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
          ...
          <span className="absolute bottom-5 left-5 rounded-full bg-moss/85 px-4 py-2 text-xs font-bold uppercase tracking-[.2em] text-cream">{pack.mood}</span>
        </div>
      </Link>
      <div className="flex flex-1 flex-col p-7">
        ...
        <div className="mt-auto pt-7">
          <Link className="btn-secondary w-full justify-between" href={`/rituales/${pack.slug}`}>Ver ritual <ArrowRight className="h-4 w-4" /></Link>
        </div>
      </div>
    </article>
  );
